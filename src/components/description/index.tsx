...
export default function Desciption({
  ...
  return (
    <div
      className={`bg-gray-900 pb-12 overflow-auto h-full p-6 text-white ${activeTab == 0 ? 'flex flex-col gap-4' : 'hidden'}`}
    >
      ...
      <div>
        <span
          className={`capitalize text-sm bg-gray-700 p-1 px-3 rounded-4xl ${data.status == 'easy' ? 'text-green-400' : data.status == 'medium' ? 'text-yellow-400' : 'text-red-400'}`}
        >
          {data.status}
        </span>
      </div>
      <div>
        {data.description.map((description, desciptionIndex) => {
          return (
            <p className="text-[18px] mb-2 text-gray-200" key={desciptionIndex}>
              {description}
            </p>
          );
        })}
